...
names(data)
### Variables used for thix example:
...
### rep_subset = indicator that participant had the repeat biomarker measure
...
### Other variables include  log intake values for percent energy from protein (PEP) analogous to those for energy.

### Note sandwich2stage and simexFit did not seem to like categorical/factor variables, so create the binary dummy variables for each race
...
#### Took 90 seconds or so to run on a macbook air
...
summary(simexFit)
plotsimexaft(simexFit,"logEn_sr",ylimit=c(-3,2),extrapolation="quadratic")

### Try linear extrapolation
### Not much better and maybe some evidence of a curve in the extrapolated estimates
...
summary(simexFitLin)
plotsimexaft(simexFitLin,"logEn_sr",ylimit=c(-3,2),extrapolation="linear")
...
### HR estimated by simex AFT model: Does not do a great job because systematic and not classical error in the self-reported energy variable
### Some bias still here, due to the error being more complicated than the simple classical error SIMEX assumes. Also had to guess at error variance.
exp(-simexFit$coef/simexFit$scale)

##########
#### Look at sensitivty to the assumed error variance.
##########
err.mat2<-.5*err.mat
